...
int			process_operator_token(t_general *ctx, int i, t_token **tokens,
				t_token **last_added);
int			process_dollar_token(t_general *ctx, int i, void **tkn_ptrs,
				int *skipped);
...
int			build_exp_str(t_general *ctx, int start, char stop_char,
				char *result);
...
void		update_existing_variable(t_env_var *env_list, char *key,
				char *value);
